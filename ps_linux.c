...
typedef struct {
    ...
    double pct_ms;
    double cpu_ms;
    long vctx, ivctx; // real OS context switches for this simulator process
} Proc;
...
int main(void) {
    ...
    printf("===== Priority Scheduling (Linux, Preemptive) =====\n");
    ...
    while (!all_done()) {
        ...
        // Print one segment per tick (simple + consistent)
    ...
    printf("PID\tPR\tAT\tBT\tCT\tTAT\tWT\tPCT(ms)\tCPU(ms)\tVCSW\tIVCSW\n");
    for (int i = 0; i < N; i++) {
        printf("%s\t%d\t%d\t%d\t%d\t%d\t%d\t%.2f\t%.2f\t%ld\t%ld\n",
               p[i].pid, p[i].pr, p[i].at, p[i].bt,
